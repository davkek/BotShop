...
async def set_cat_title_handler( query: CallbackQuery, callback_data: dict, state: FSMContext):

    category = query.message.text
    idx = md5(category.encode("utf-8")).hexdigest()
    category_idx = callback_data['id']
    db.query("INSERT INTO categories VALUES (?, ?, ?)", (idx, category, category_idx))
